...
fn main() {
    ...
    // //ok so I think tx = sender (transmitter), rx = receiver
    // //unbounde channel = simply means it will buffer as many values as it can until it runs out of memory
    ...
    // //technically isn't necessary because the main thread would never get past the receiving loop, until all transmitting sites are closed
    ...
    let handle_a = thread::spawn(move || {
        for msg in rx {
            println!("A thread: Received {}", msg);
        }
    });

    let handle_b = thread::spawn(move || {
        for msg in rx2 {
            println!("B thread: Received {}", msg);
        }
    });
